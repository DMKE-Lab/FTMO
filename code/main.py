...
class Model_Run(object):
	...
	def build_graph(self, max_=50):
		...
		# with open(self.datapath + '/other_tasks.json') as f:
		# 	# lines = f.readlines()
		# 	for line in tqdm(lines):
		# 		e1,rel,e2 = line.rstrip().split()
		# with open(self.datapath + '/other_tasks.json') as f:
		other_tasks = json.load(open(self.datapath + '/other_tasks.json'))
		for task in other_tasks.keys():
			for quad in other_tasks[task]:
				...
				e2 = quad[2]
				# print(rel)
				# print(self.e1_rele2[e2])
				# self.e1_rele2[e1].append((self.symbol2id[rel], self.symbol2id[e2]))
				# self.e1_rele2[e2].append((self.symbol2id[rel+'_inv'], self.[e1]))
